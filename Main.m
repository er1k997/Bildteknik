...
% imshow(result);
% 
% score_piqe_log=piqe(result);
% score_br_log = brisque(result);
% score_niqe_log = niqe(result);
...
%------- Salt and Pepper Laplace -----%
ailimg_noise = own_Laplace(noise_img);
figure;
imshow(ailimg_noise);
title('Laplace with Salt & Pepper Noise');

score_piqe_ail_noise = piqe(ailimg_noise);
score_br_ail_noise = brisque(ailimg_noise);
score_niqe_ail_noise = niqe(ailimg_noise);
disp("Piqe (Laplace Noise): " + score_piqe_ail_noise);
disp("Brisque (Laplace Noise): " + score_br_ail_noise);
disp("Niqe (Laplace Noise): " + score_niqe_ail_noise);
