function [edofimg, varargout] = AIL(img,varargin)
...
dilatesize = checkparam('dilatesize', 41);
blendsize = checkparam('blendsize', 31);
blendstd = checkparam('blendstd', 3);
...
for ii = 1:length(img) 
    ...
    laplacian1 = abs(del2(single(img{ii})));                        % Fine scale
    laplacian2 = abs(del2(imgaussfilt(single(img{ii}), 1)));        % Medium scale
    laplacian3 = abs(del2(imgaussfilt(single(img{ii}), 2)));        % Coarse scale
    
    % Combine multi-scale Laplacian responses
    imgfiltered{ii} = max(cat(3, laplacian1, laplacian2, laplacian3), [], 3);
